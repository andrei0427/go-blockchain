...
import (
	...
	"github.com/andrei0427/go-blockchain/util"
	"github.com/stretchr/testify/assert"
)

func TestTxMaxLength(t *testing.T) {
	p := NewTxPool(1)
	p.Add(util.NewRandomTransaction(30))
	...
	p.Add(tx)
	// tx should replace the existing transaction due to maxLength of 1
	assert.Equal(t, 1, p.all.Count())
